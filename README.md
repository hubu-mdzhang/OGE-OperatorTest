...
2. 检查持久化登录态和完整页面契约。
3. 重载工作区，保存运行前 Globe。
4. 通过 Monaco Model API 写入完整代码；真实页面未暴露 `window.monaco` 时改用合成拖放注入——Monaco 的 drop 处理按原文插入，可绕过键盘输入触发的自动缩进与自动补全括号（键盘路径实测会改写代码内容，粘贴在真实页面上不可用）。
5. 注入校验：Model API 路径逐字符回读比对；拖放路径全选后读取 textarea 镜像——短代码全文逐字符比对，长代码镜像被 Monaco 截断为「前缀+…+后缀」，按省略号切分后分别比对两端；运行后 `executeCode` 载荷逐字符比对是最终权威。
